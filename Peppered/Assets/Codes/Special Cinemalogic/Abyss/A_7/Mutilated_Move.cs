...
public class Mutilated_Move : MonoBehaviour
{
    ...
    private void MoveThisGuy()
    {
        ...
        if (Input.GetAxisRaw("Horizontal") == 0) { MyAnim.Play("Mutilated_Idle"); }
    }

    private void TurnSprite()
    {
        //Turn Sprite Around
        if (Input.GetAxisRaw("Horizontal") > 0 && MySceneManager.CutscenePlaying == false) { MyRen.flipX = false; Player.gameObject.transform.localScale = new Vector3(1, 1, 1); MyAnim.Play("Mutlated_Move"); }
        else if (Input.GetAxisRaw("Horizontal") < 0 && MySceneManager.CutscenePlaying == false) { MyRen.flipX = true; Player.gameObject.transform.localScale = new Vector3(-1, 1, 1); MyAnim.Play("Mutlated_Move"); }
    }
}
